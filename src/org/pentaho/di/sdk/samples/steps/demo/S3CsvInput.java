...
public class S3CsvInput extends BaseStep implements StepInterface {
	...
	private boolean openNextFile() throws KettleException {
		try {
			...
			// If we are running in parallel we only want to grab a part of the content, not
			// everything.
			//
			if (data.parallel) {

				data.s3Object = data.s3Service.getObject(data.s3bucket, data.filenames[data.filenr], null, null, null,
						null, data.bytesToSkipInFirstFile,
			...
			if ((meta.isHeaderPresent() && !data.parallel) || // Standard flat file : skip header
					...
					|| (data.parallel && data.filenr > data.startFilenr && data.bytesToSkipInFirstFile <= 0) // parallel
																												// processing
																												// :
																												// start
																												// of
	...
	public void dispose(StepMetaInterface smi, StepDataInterface sdi) {
		...
		S3CsvInputMeta meta = (S3CsvInputMeta) smi;
		S3CsvInputData data = (S3CsvInputData) sdi;
